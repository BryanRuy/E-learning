...
export async function UpdateBasicInfo(
  ...
  try {
    ...
    if (!_userId || role != "super-admin")
      throw new UnAuthorizedError(
        ...a("You are not authorized to access this resource")
      );

    let superAdminCore = UserFactory("super-admin");
    let superAdmin = (await superAdminCore.GetUserByUuid({
      key: userId,
    ...
    let user = await ProfileUtil.UpdateBasicInfo(
      params,
      superAdmin._userId!,
      superAdmin.roleId
    );
...
export async function UpdatePassword(
  ...
  try {
    ...
    if (!newPassword) throw new BadRequestError(...a("Enter a valid password"));
    ...
    const user = await ProfileUtil.UpdatePassword(
      newPassword,
      currentPassword,
      superAdmin,
      superAdmin.roleId!
...
export async function UpdateProfilePicture(
  ...
  try {
    ...
    if (!req.file) throw new BadRequestError(...a("Select an image first"));
    ...
    const user = await ProfileUtil.UpdateUserProfilePicture(
...
export async function UpdateUsersPasswordBySuperAdmin(
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    const updatedUser = await ProfileUtil.UpdateUserPasswordBySuperAdmin(
      newPassword,
      user,
      user.roleId!
    );

    return LocaleDataResponse(res, 200, updatedUser);
  } catch (err) {
    next(err);
  }
}
